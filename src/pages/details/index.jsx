...
export default function Details(){
    ...
    return <div className="container mx-auto py-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <div className="flex flex-col gap-3">
            <span className="text-sm text-cyan-800 font-medium">    {recipeDetailsData?.recipe?.publisher}  </span>
            <h3 className="font-bold text-2xl truncate text-black"> {recipeDetailsData?.recipe?.title}  </h3>

            <div>
                <button onClick={() => handleAddToFavorites(recipeDetailsData?.recipe)} className="p-3 px-8 rounded-lg text-sm uppercase font-medium tracking-wider mt-3 inline-block shadow-md text-white bg-black ">
                    {
